...
export const ShopProvider = ({ children }) => {
  ...
  const [businessSettings, setBusinessSettings] = useState({
    ...
    returnPolicy: 'No returns or exchanges once shipped, unless damaged in transit.',
    supportNumber: '+91 89253 25330'
  });
  ...
  useEffect(() => {
    import('firebase/firestore').then(({ doc, onSnapshot }) => {
      const unsubscribe = onSnapshot(doc(db, "settings", "business"), (docSnap) => {
        if (docSnap.exists()) {
          setBusinessSettings(prev => ({ ...prev, ...docSnap.data() }));
        }
      });
      return () => unsubscribe();
    });
